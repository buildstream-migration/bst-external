...
class BazelSource(Source):
    ...
    def configure(self, node):
        self.node_validate(node, ['workspace-dir', 'allow-host-bazel', 'repo-file', 'targets', 'ref'] +
                           Source.COMMON_CONFIG_KEYS)

        # Whether to allow falling back to a bazel installed on the host to generate
        # a "repository resolved file"
        #
        self.allow_host_bazel = self.node_get_member(node, bool, 'allow-host-bazel', default=False)

        # The path to the "repository resolved file". Defaults to .bst-sources
        self.repo_file = self.node_get_member(node, str,'repo-file', default='.bst-sources')
        ...
        #
        self.targets = self.node_get_member(node, list, 'targets', default=[])

        # The path to the bazel workspace. This can be used for repos which have
        # the bazel project in a subdirectory.
        #
        self.workspace_dir = self.node_get_member(node, str, 'workspace-dir', default='.')

        # The unique ref of this source. The SHA256 of the repository resolved file
        self.ref = self.node_get_member(node, str, 'ref', default=None)
