...
# Block size used when reading tarball for hash
BLOCKSIZE = 65536
...
def test_tar_build(cli, datafiles):
    ...
    result = cli.run(project=project, args=['build', 'tar/tar-test.bst'])
    result.assert_success()

    result = cli.run(project=project, args=['checkout', 'tar/tar-test.bst',
        checkout_dir])
    result.assert_success()
...
def test_tar_checksums(cli, datafiles):
    project = str(datafiles)
    checkout_dir = os.path.join(cli.directory, 'tar_chackout')
    tarpath = os.path.join(checkout_dir, 'hello.tar.gz')

    result = cli.run(project=project, args=['build', 'tar/tar-test-checksums.bst'])
    result.assert_success()

    result = cli.run(project=project, args=['checkout',
                     'tar/tar-test-checksums.bst', checkout_dir])
    result.assert_success()

    assert_contains(checkout_dir, ["/hello.tar.gz", "/hello.sha256sum",
                                   "/hello.md5sum"])
